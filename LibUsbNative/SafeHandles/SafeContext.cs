...
internal sealed class SafeContext : SafeHandle, ISafeContext
{
    ...
    {
        var result = LibUsb.Api.libusb_init(out var raw);
    ...
    {
        var rc = LibUsb.Api.libusb_set_option(handle, option, value);
    ...
    public LibUsbError HandleEventsCompleted(IntPtr completedPtr)
    {
        ...
        return LibUsb.Api.libusb_handle_events_completed(handle, completedPtr);
    }

    public void InterruptEventHandler()
    {
        LibUsb.Api.libusb_interrupt_event_handler(handle);
    }
    ...
    )
    {
        ...
        var result = LibUsb.Api.libusb_hotplug_register_callback(
            handle,
            events,
            flags,
            vendorId,
    ...
    public void HotplugDeregisterCallback(IntPtr callbackHandle)
    {
        ...
        LibUsb.Api.libusb_hotplug_deregister_callback(handle, callbackHandle);
    }

    public (ISafeDeviceList, uint) GetDeviceList()
    {
        var rc = LibUsb.Api.libusb_get_device_list(handle, out var list);
        ...
        if (!success)
        {
            LibUsb.Api.libusb_free_device_list(list, 1);
